...
// Compatible with libsodium crypto_aead_xchacha20poly1305_ietf_*
...
namespace keylock::crypto::aead_xchacha20poly1305 {

    // Constants matching libsodium
    inline constexpr size_t KEYBYTES = 32;
    inline constexpr size_t NPUBBYTES = 24; // nonce size
    ...
    namespace detail {
        ...
        // Compute Poly1305 key from ChaCha20
        inline void derive_poly_key(uint8_t poly_key[32], const uint8_t key[32], const uint8_t nonce[24]) {
            // First 32 bytes of XChaCha20 keystream
            std::memset(poly_key, 0, 32);
            chacha20::xchacha20(poly_key, poly_key, 32, key, nonce, 0);
        }
    ...
    inline int encrypt(uint8_t *c, unsigned long long *clen_p, const uint8_t *m, unsigned long long mlen,
                       ...
                       const uint8_t *nsec, // unused, for API compatibility
        ...
        // Encrypt message (skip first 64 bytes of keystream used for poly key)
        chacha20::xchacha20(c, m, mlen, key, nonce, 1);
    ...
    inline int decrypt(uint8_t *m, unsigned long long *mlen_p,
        ...
        // Decrypt
        chacha20::xchacha20(m, c, mlen, key, nonce, 1);
    ...
    // Convenience: detached encryption (separate tag output)
    ...
    // Convenience: detached decryption
    inline int decrypt_detached(uint8_t *m, uint8_t *nsec, const uint8_t *c, unsigned long long clen,
        ...
        chacha20::xchacha20(m, c, clen, key, nonce, 1);
