#pragma once

// AES block cipher (256-bit key)
// Adapted from plusaes (Boost Software License 1.0)

#include <cstddef>
#include <cstdint>
#include <cstring>
#include <vector>

namespace keylock::crypto::aes256 {

    inline constexpr size_t KEYBYTES = 32;
    inline constexpr size_t BLOCKBYTES = 16;

    namespace detail {

        inline constexpr uint8_t sbox[256] = {
            };

        inline constexpr uint32_t rcon[11] = {0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};

        struct State {
            uint32_t w[4];
            uint32_t &operator[](int index) { return w[index]; }
            const uint32_t &operator[](int index) const { return w[index]; }
        };

        using RoundKey = State;

        inline uint32_t sub_word(uint32_t w) {
            return sbox[(w >> 0) & 0xFF] << 0 | sbox[(w >> 8) & 0xFF] << 8 | sbox[(w >> 16) & 0xFF] << 16 |
                   sbox[(w >> 24) & 0xFF] << 24;
        }

        inline uint32_t rot_word(uint32_t v) { return ((v >> 8) & 0x00FFFFFF) | ((v & 0xFF) << 24); }

        inline uint8_t mul2(uint8_t b) {
            uint8_t m2 = b << 1;
            if (b & 0x80) {
                m2 ^= 0x1B;
            }
            return m2;
        }

        inline void add_round_key(const RoundKey &key, State &state) {
            for (int i = 0; i < 4; ++i) {
                state[i] ^= key[i];
            }
        }

        inline void sub_bytes(State &state) {
            for (int i = 0; i < 4; ++i) {
                state[i] = sub_word(state[i]);
            }
        }

        inline void shift_rows(State &state) {
            const State ori = {state[0], state[1], state[2], state[3]};
            for (int r = 1; r < 4; ++r) {
                uint32_t m2 = 0xFF << (r * 8);
                uint32_t m1 = ~m2;
                for (int c = 0; c < 4; ++c) {
                    state[c] = (state[c] & m1) | (ori[(c + r) % 4] & m2);
                }
            }
        }

        inline void mix_columns(State &state) {
            for (int i = 0; i < 4; ++i) {
                uint8_t v0_1 = (state[i] >> 0) & 0xFF;
                uint8_t v1_1 = (state[i] >> 8) & 0xFF;
                uint8_t v2_1 = (state[i] >> 16) & 0xFF;
                uint8_t v3_1 = (state[i] >> 24) & 0xFF;

                uint8_t v0_2 = mul2(v0_1);
                uint8_t v1_2 = mul2(v1_1);
                uint8_t v2_2 = mul2(v2_1);
                uint8_t v3_2 = mul2(v3_1);

                uint8_t v0_3 = v0_2 ^ v0_1;
                uint8_t v1_3 = v1_2 ^ v1_1;
                uint8_t v2_3 = v2_2 ^ v2_1;
                uint8_t v3_3 = v3_2 ^ v3_1;

                state[i] = (v0_2 ^ v1_3 ^ v2_1 ^ v3_1) << 0 | (v0_1 ^ v1_2 ^ v2_3 ^ v3_1) << 8 |
                           (v0_1 ^ v1_1 ^ v2_2 ^ v3_3) << 16 | (v0_3 ^ v1_1 ^ v2_1 ^ v3_2) << 24;
            }
        }

        inline void copy_bytes_to_state(const uint8_t data[16], State &state) {
            std::memcpy(&state[0], data + 0, 4);
            std::memcpy(&state[1], data + 4, 4);
            std::memcpy(&state[2], data + 8, 4);
            std::memcpy(&state[3], data + 12, 4);
        }

        inline void copy_state_to_bytes(const State &state, uint8_t buf[16]) {
            std::memcpy(buf + 0, &state[0], 4);
            std::memcpy(buf + 4, &state[1], 4);
            std::memcpy(buf + 8, &state[2], 4);
            std::memcpy(buf + 12, &state[3], 4);
        }

    } // namespace detail

    struct Context {
        detail::RoundKey rkeys[15]; // 14 rounds + 1 for AES-256
        size_t rounds;
    };

    inline void key_setup(Context *ctx, const uint8_t key[32]) {
        constexpr int nk = 8;  // AES-256: 8 words
        constexpr int nr = 14; // AES-256: 14 rounds
        constexpr int nb = 4;

        ctx->rounds = nr;

        uint32_t w[nb * (nr + 1)];
        for (int i = 0; i < nk; ++i) {
            std::memcpy(&w[i], key + i * 4, 4);
        }

        for (int i = nk; i < nb * (nr + 1); ++i) {
            uint32_t t = w[i - 1];
            if (i % nk == 0) {
                t = detail::sub_word(detail::rot_word(t)) ^ detail::rcon[i / nk];
            } else if (i % nk == 4) {
                t = detail::sub_word(t);
            }
            w[i] = t ^ w[i - nk];
        }

        for (int i = 0; i <= nr; ++i) {
            std::memcpy(&ctx->rkeys[i], &w[i * nb], nb * 4);
        }
    }

    inline void encrypt_block(const Context *ctx, const uint8_t in[16], uint8_t out[16]) {
        detail::State s;
        detail::copy_bytes_to_state(in, s);

        detail::add_round_key(ctx->rkeys[0], s);

        for (size_t i = 1; i < ctx->rounds; ++i) {
            detail::sub_bytes(s);
            detail::shift_rows(s);
            detail::mix_columns(s);
            detail::add_round_key(ctx->rkeys[i], s);
        }

        detail::sub_bytes(s);
        detail::shift_rows(s);
        detail::add_round_key(ctx->rkeys[ctx->rounds], s);

        detail::copy_state_to_bytes(s, out);
    }

} // namespace keylock::crypto::aes256
