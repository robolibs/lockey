...
// ISO/IEC 10118-3:2004 compliant
...
namespace keylock::hash::whirlpool {
    ...
    namespace detail {
        ...
        inline constexpr uint64_t RC[10] = {0x4f01b887e8c62318ull, 0x52916f79f5d2a636ull, 0x357b0ca38e9bbc60ull,
        ...
        template <int a> inline uint64_t G(const uint64_t *ll) {
            return T[0][static_cast<uint8_t>(ll[a % 8])] ^ T[1][static_cast<uint8_t>(ll[(a + 7) % 8] >> 8)] ^
                   T[2][static_cast<uint8_t>(ll[(a + 6) % 8] >> 16)] ^
                   T[3][static_cast<uint8_t>(ll[(a + 5) % 8] >> 24)] ^
                   T[4][static_cast<uint8_t>(ll[(a + 4) % 8] >> 32)] ^
                   T[5][static_cast<uint8_t>(ll[(a + 3) % 8] >> 40)] ^
                   T[6][static_cast<uint8_t>(ll[(a + 2) % 8] >> 48)] ^
                   T[7][static_cast<uint8_t>(ll[(a + 1) % 8] >> 56)];
        }
    ...
    namespace detail {
        ...
        inline void transform(Context *ctx, const uint8_t *block) {
            ...
            std::memcpy(K, ctx->H, sizeof(K));
            for (int i = 0; i < 8; ++i) {
                state[i] = ctx->H[i] ^ reinterpret_cast<const uint64_t *>(block)[i];
            }
            ...
            for (int r = 0; r < 10; ++r) {
                ...
                std::memcpy(state, L, sizeof(L));
            }

            for (int i = 0; i < 8; ++i) {
                ctx->H[i] ^= state[i] ^ reinterpret_cast<const uint64_t *>(block)[i];
            }
        }
    ...
    inline void final(Context *ctx, uint8_t hash[64]) {
        ...
        uint64_t mlen = detail::byteswap(total_bits);
        ...
        std::memcpy(ctx->buffer + 56, &mlen, 8);
        detail::transform(ctx, ctx->buffer);

        std::memcpy(hash, ctx->H, 64);
