...
// Russian GOST standard, 256 and 512 bit variants
...
namespace keylock::hash::streebog {
    ...
    namespace detail {
        ...
        const uint64_t inline constexpr T[8][256] = {
        ...
        template <int I> inline uint64_t lps(const uint8_t *ch) {
            return T[0][ch[I]] ^ T[1][ch[8 + I]] ^ T[2][ch[16 + I]] ^ T[3][ch[24 + I]] ^ T[4][ch[32 + I]] ^
                   T[5][ch[40 + I]] ^ T[6][ch[48 + I]] ^ T[7][ch[56 + I]];
        }

        inline void lps_block(uint64_t *block) {
            uint8_t ch[64];
            std::memcpy(ch, block, 64);
            block[0] = lps<0>(ch);
            block[1] = lps<1>(ch);
            block[2] = lps<2>(ch);
            block[3] = lps<3>(ch);
        ...
        inline void xor_blocks(uint64_t *block, const uint64_t *toxor) {
            for (int i = 0; i < 8; ++i)
                block[i] ^= toxor[i];
        }
        ...
        inline void gN(uint64_t *h, const uint8_t *m, uint64_t N) {
            uint64_t hN[8];
            std::memcpy(hN, h, 64);
            hN[0] ^= N;
            lps_block(hN);
    ...
    struct Context {
        ...
        size_t buffer_len;
        uint64_t total;
        size_t hash_size; // 256 or 512
    };

    inline void init(Context *ctx, size_t hash_size = 512) {
        std::memset(ctx->H, (hash_size == 512) ? 0 : 1, sizeof(ctx->H));
    ...
    inline void update(Context *ctx, const uint8_t *message, size_t message_size) {
        if (ctx->buffer_len > 0) {
            ...
            if (ctx->buffer_len == 64) {
                detail::gN(ctx->H, ctx->buffer, ctx->total);
                detail::addm(ctx->buffer, ctx->S);
                ctx->total += 512;
                ctx->buffer_len = 0;
            }
        }
        ...
        while (message_size >= 64) {
            detail::gN(ctx->H, message, ctx->total);
            detail::addm(message, ctx->S);
            ctx->total += 512;
            message += 64;
            message_size -= 64;
        }
    ...
    inline void final(Context *ctx, uint8_t *hash) {
        ...
        detail::gN(ctx->H, ctx->buffer, ctx->total);
        detail::addm(ctx->buffer, ctx->S);
        ctx->total += (ctx->buffer_len - 1) * 8;

        uint8_t len_block[64] = {0};
        std::memcpy(len_block, &ctx->total, sizeof(ctx->total));
        detail::gN(ctx->H, len_block, 0);
        detail::gN(ctx->H, reinterpret_cast<const uint8_t *>(ctx->S), 0);

        size_t out_bytes = ctx->hash_size / 8;
        std::memcpy(hash, reinterpret_cast<uint8_t *>(ctx->H) + (64 - out_bytes), out_bytes);

        crypto::constant_time::wipe(ctx, sizeof(*ctx));
    }

    inline void hash256(uint8_t hash[32], const uint8_t *message, size_t message_size) {
        Context ctx;
        init(&ctx, 256);
    ...
    inline void hash512(uint8_t hash[64], const uint8_t *message, size_t message_size) {
        Context ctx;
        init(&ctx, 512);
        update(&ctx, message, message_size);
        final(&ctx, hash);
