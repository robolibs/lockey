...
namespace keylock::cert {
    ...
    namespace {
        ...
        std::string encode_base64(ByteSpan data) {
            std::string out;
        ...
        std::string strip_whitespace(std::string_view view, size_t start, size_t end) {
        ...
        PemResult error_result(std::string message) { return PemResult{false, {}, std::move(message)}; }

        std::string build_block(std::string_view label, std::string body, size_t line_length) {
    ...
    PemResult pem_decode(std::string_view pem, std::optional<std::string_view> expected_label) {
        const auto begin_pos = pem.find(kBeginMarker);
        if (begin_pos == std::string_view::npos) {
            return error_result("missing PEM BEGIN marker");
        }

        const size_t label_start = begin_pos + kBeginMarker.size();
        const auto label_end = pem.find(kTrailer, label_start);
        if (label_end == std::string_view::npos) {
            return error_result("unterminated PEM header");
        }
        const std::string label(pem.substr(label_start, label_end - label_start));

        if (expected_label && label != *expected_label) {
            return error_result("unexpected PEM label");
        }

        size_t content_start = pem.find_first_of("\r\n", label_end + kTrailer.size());
        if (content_start == std::string_view::npos) {
            return error_result("PEM header missing newline");
        }
        while (content_start < pem.size() && (pem[content_start] == '\r' || pem[content_start] == '\n')) {
            ++content_start;
        }

        const std::string end_label = std::string(kEndMarker) + label + std::string(kTrailer);
        const auto footer_pos = pem.find(end_label, content_start);
        if (footer_pos == std::string_view::npos) {
            return error_result("missing PEM END marker");
        }

        const auto base64_data = strip_whitespace(pem, content_start, footer_pos);
        if (base64_data.empty()) {
            return error_result("empty PEM body");
        }

        std::vector<uint8_t> decoded;
        if (!decode_base64(base64_data, decoded)) {
            return error_result("invalid base64 content");
        }
    ...
    std::string pem_encode(ByteSpan der, std::string_view label, size_t line_length) {
        auto body = encode_base64(der);
        return build_block(label, std::move(body), line_length);
    }

    PemResult pem_decode_certificate(std::string_view pem) { return pem_decode(pem, "CERTIFICATE"); }

    PemResult pem_decode_private_key(std::string_view pem) { return pem_decode(pem, "PRIVATE KEY"); }

    std::string pem_encode_certificate(ByteSpan der) { return pem_encode(der, "CERTIFICATE"); }

    std::string pem_encode_private_key(ByteSpan der) { return pem_encode(der, "PRIVATE KEY"); }
