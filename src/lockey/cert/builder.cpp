...
namespace lockey::cert {
    ...
    namespace {
        ...
        std::vector<uint8_t> make_random_serial() {
            lockey::utils::ensure_sodium_init();
        ...
        Oid get_extension_oid(ExtensionId id) {
        ...
        Oid get_signature_oid(SignatureAlgorithmId id) {
        ...
        bool is_utctime(const std::tm &tm_value) {
            const int year = tm_value.tm_year + 1900;
            return year >= 1950 && year <= 2049;
        }

        std::tm to_gmtime(std::chrono::system_clock::time_point tp) {
    ...
    CertificateBuilder &CertificateBuilder::set_version(int version) {
        version_ = std::clamp(version, 1, 3);
        return *this;
    }

    CertificateBuilder &CertificateBuilder::set_serial(const std::vector<uint8_t> &serial) {
    ...
    CertificateBuilder &CertificateBuilder::set_serial(uint64_t serial) {
    ...
    CertificateBuilder &CertificateBuilder::set_signature_algorithm(SignatureAlgorithmId algorithm,
    ...
    CertificateBuilder &CertificateBuilder::set_issuer(const DistinguishedName &dn) {
        issuer_ = dn;
        issuer_explicit_ = true;
        return *this;
    }

    CertificateBuilder &CertificateBuilder::set_subject(const DistinguishedName &dn) {
        subject_ = dn;
        subject_explicit_ = true;
        return *this;
    }

    CertificateBuilder &CertificateBuilder::set_subject_from_string(std::string_view dn) {
    ...
    CertificateBuilder &CertificateBuilder::set_issuer_from_string(std::string_view dn) {
        auto parsed = DistinguishedName::from_string(dn);
        if (parsed.success) {
    ...
    CertificateBuilder &CertificateBuilder::set_validity(std::chrono::system_clock::time_point not_before,
    ...
    CertificateBuilder &CertificateBuilder::set_subject_public_key_info(const SubjectPublicKeyInfo &spki) {
        subject_public_key_info_ = spki;
        return *this;
    }

    CertificateBuilder &CertificateBuilder::set_subject_public_key_ed25519(const std::vector<uint8_t> &public_key) {
    ...
    CertificateBuilder &CertificateBuilder::add_extension(const RawExtension &extension) {
    ...
    CertificateBuilder &CertificateBuilder::set_basic_constraints(bool is_ca, std::optional<uint32_t> path_length,
                                                                  bool critical) {
        return add_extension(build_basic_constraints_extension(is_ca, path_length, critical));
    }

    CertificateBuilder &CertificateBuilder::set_key_usage(uint16_t bits, bool critical) {
    ...
    CertificateBuilder &CertificateBuilder::set_subject_key_identifier(const std::vector<uint8_t> &key_id,
                                                                       bool critical) {
        return add_extension(build_key_identifier_extension(key_id, critical, ExtensionId::SubjectKeyIdentifier));
    }

    CertificateBuilder &CertificateBuilder::set_authority_key_identifier(const std::vector<uint8_t> &key_id,
                                                                         bool critical) {
        return add_extension(build_key_identifier_extension(key_id, critical, ExtensionId::AuthorityKeyIdentifier));
    }

    std::optional<std::string> CertificateBuilder::validate_inputs(bool self_signed) const {
    ...
    CertificateResult<Certificate> CertificateBuilder::build(const crypto::Lockey::KeyPair &issuer_key,
    ...
    CertificateResult<Certificate> CertificateBuilder::build_ed25519(const crypto::Lockey::KeyPair &issuer_key,
                                                                     bool self_signed) const {
        ...
        auto serial = serial_number_.empty() ? make_random_serial() : serial_number_;

        CertificateBuilder builder_copy(*this);
        builder_copy.serial_number_ = serial;

        auto issuer_dn = self_signed ? builder_copy.subject_
    ...
    RawExtension CertificateBuilder::build_basic_constraints_extension(bool is_ca, std::optional<uint32_t> path_length,
                                                                       bool critical) const {
        ...
        ext.oid = get_extension_oid(ExtensionId::BasicConstraints);
        ext.critical = critical;
        ext.value = seq;
        return ext;
    }

    RawExtension CertificateBuilder::build_key_usage_extension(uint16_t bits, bool critical) const {
        uint16_t value = bits;
        std::vector<uint8_t> buffer = {static_cast<uint8_t>((value >> 8) & 0xFFU), static_cast<uint8_t>(value & 0xFFU)};
    ...
    RawExtension
    CertificateBuilder::build_subject_alt_name_extension(const std::vector<SubjectAltNameExtension::GeneralName> &names,
                                                         bool critical) const {
        auto encoded = encode_general_names(names);
        RawExtension ext{};
        ext.id = ExtensionId::SubjectAltName;
        ext.oid = get_extension_oid(ExtensionId::SubjectAltName);
        ext.critical = critical;
        ext.value = der::encode_sequence(encoded);
        return ext;
    }

    RawExtension CertificateBuilder::build_key_identifier_extension(const std::vector<uint8_t> &key_id, bool critical,
                                                                    ExtensionId id) const {
        auto value = der::encode_octet_string(ByteSpan(key_id.data(), key_id.size()));
        RawExtension ext{};
        ext.id = id;
        ext.oid = get_extension_oid(id);
        ext.critical = critical;
        ext.value = value;
        return ext;
    }

    std::vector<uint8_t> CertificateBuilder::encode_tbs_certificate(const DistinguishedName &issuer_dn) const {
        std::vector<std::vector<uint8_t>> fields;
        ...
        auto serial_value = serialize_serial(serial_number_.empty() ? make_random_serial() : serial_number_);
    ...
    std::vector<uint8_t> CertificateBuilder::encode_algorithm_identifier(const AlgorithmIdentifier &alg) const {
        auto oid = get_signature_oid(alg.signature);
    ...
    std::vector<uint8_t> CertificateBuilder::encode_validity(const Validity &validity) const {
        std::vector<std::vector<uint8_t>> fields;
    ...
    std::vector<uint8_t> CertificateBuilder::encode_subject_public_key_info(const SubjectPublicKeyInfo &spki) const {
        std::vector<std::vector<uint8_t>> fields;
    ...
    std::vector<uint8_t> CertificateBuilder::encode_extensions(const std::vector<RawExtension> &extensions) const {
    ...
    std::vector<uint8_t>
    CertificateBuilder::encode_general_names(const std::vector<SubjectAltNameExtension::GeneralName> &names) const {
        std::vector<std::vector<uint8_t>> encoded;
    ...
    std::vector<uint8_t> CertificateBuilder::format_time(std::chrono::system_clock::time_point tp, bool utc_time) {
        auto tm_value = to_gmtime(tp);
    ...
    std::vector<uint8_t> CertificateBuilder::serialize_time(std::chrono::system_clock::time_point tp) {
        auto tm_value = to_gmtime(tp);
        const bool utc = is_utctime(tm_value);
        return format_time(tp, utc);
    }

    std::vector<uint8_t> CertificateBuilder::serialize_serial(const std::vector<uint8_t> &serial) {
        if (serial.empty()) {
            auto random_serial = make_random_serial();
            return der::encode_integer(random_serial);
        }
        return der::encode_integer(serial);
    }

    std::vector<uint8_t> CertificateBuilder::encode_extension(const RawExtension &extension) {
        std::vector<std::vector<uint8_t>> fields;
    ...
    std::vector<uint8_t> CertificateBuilder::encode_ip_address(const std::string &ip) {
