...
ASN1Result<int> parse_version(ByteSpan input) {
    ...
    if (identifier.tag_class != ASN1Class::ContextSpecific || identifier.tag_number != 0 || !identifier.constructed) {
...
ASN1Result<std::vector<RawExtension>> parse_extensions(ByteSpan input) {
    ...
    const auto &identifier = header.value.identifier;
    if (identifier.tag_class != ASN1Class::ContextSpecific || identifier.tag_number != 3 || !identifier.constructed) {
...
ParseResult parse_x509_cert(ByteSpan input) {
    return parse_certificate(input, false);
}

ParseResult parse_x509_cert_relaxed(ByteSpan input) {
    return parse_certificate(input, true);
}
