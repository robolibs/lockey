#include <lockey/cert/crl_builder.hpp>

#include <lockey/cert/asn1_writer.hpp>
#include <lockey/cert/oid_registry.hpp>
#include <lockey/utils/sodium_utils.hpp>

namespace lockey::cert {

CrlBuilder &CrlBuilder::set_issuer(const DistinguishedName &dn) {
    issuer_ = dn;
    issuer_set_ = true;
    return *this;
}

CrlBuilder &CrlBuilder::set_issuer_from_string(std::string_view dn) {
    auto parsed = DistinguishedName::from_string(dn);
    if (parsed.success) {
        issuer_ = parsed.value;
        issuer_set_ = true;
    }
    return *this;
}

CrlBuilder &CrlBuilder::set_this_update(std::chrono::system_clock::time_point tp) {
    this_update_ = tp;
    return *this;
}

CrlBuilder &CrlBuilder::set_next_update(std::chrono::system_clock::time_point tp) {
    next_update_ = tp;
    return *this;
}

CrlBuilder &CrlBuilder::add_revoked(const RevokedCertificate &entry) {
    entries_.push_back(entry);
    return *this;
}

CrlBuilder &CrlBuilder::add_revoked(std::vector<uint8_t> serial, std::chrono::system_clock::time_point when,
                                    std::optional<CrlReason> reason,
                                    std::optional<std::chrono::system_clock::time_point> invalidity) {
    RevokedCertificate entry{};
    entry.serial_number = std::move(serial);
    entry.revocation_date = when;
    entry.reason = reason;
    entry.invalidity_date = invalidity;
    entries_.push_back(std::move(entry));
    return *this;
}

std::optional<std::string> CrlBuilder::validate_inputs() const {
    if (!issuer_set_) {
        return "issuer not set";
    }
    if (this_update_ == std::chrono::system_clock::time_point{}) {
        return "thisUpdate not set";
    }
    return std::nullopt;
}

std::vector<uint8_t> CrlBuilder::encode_revoked_entries() const {
    std::vector<std::vector<uint8_t>> entries;
    for (const auto &revoked : entries_) {
        std::vector<std::vector<uint8_t>> fields;
        fields.push_back(der::encode_integer(revoked.serial_number));
        fields.push_back(der::serialize_time(revoked.revocation_date));
        std::vector<std::vector<uint8_t>> extensions;
        if (revoked.reason) {
            std::vector<uint8_t> reason_octet{static_cast<uint8_t>(*revoked.reason)};
            auto reason = der::encode_octet_string(ByteSpan(reason_octet.data(), reason_octet.size()));
            extensions.push_back(der::encode_sequence(reason));
        }
        if (revoked.invalidity_date) {
            auto time = der::encode_generalized_time(der::format_time(*revoked.invalidity_date, false));
            extensions.push_back(der::encode_sequence(time));
        }
        if (!extensions.empty()) {
            fields.push_back(der::encode_sequence(der::concat(extensions)));
        }
        entries.push_back(der::encode_sequence(der::concat(fields)));
    }
    return der::encode_sequence(der::concat(entries));
}

std::vector<uint8_t> CrlBuilder::encode_tbs() const {
    std::vector<std::vector<uint8_t>> fields;
    auto version = der::encode_integer(2);
    fields.push_back(
        der::encode_tlv(ASN1Class::ContextSpecific, true, 0, ByteSpan(version.data(), version.size())));
    AlgorithmIdentifier alg{};
    alg.signature = SignatureAlgorithmId::Ed25519;
    auto sig_oid = oid_for_signature(alg.signature);
    if (!sig_oid) {
        return {};
    }
    fields.push_back(der::encode_sequence(der::encode_oid(*sig_oid)));
    fields.push_back(issuer_.der());
    fields.push_back(der::serialize_time(this_update_));
    if (next_update_) {
        fields.push_back(der::serialize_time(*next_update_));
    }
    if (!entries_.empty()) {
        fields.push_back(encode_revoked_entries());
    }
    return der::encode_sequence(der::concat(fields));
}

CertificateResult<Crl> CrlBuilder::build(const crypto::Lockey::KeyPair &issuer_key) const {
    if (auto validation = validate_inputs()) {
        return CertificateResult<Crl>::failure(*validation);
    }

    auto tbs = encode_tbs();
    crypto::Lockey signer(crypto::Lockey::Algorithm::Ed25519);
    auto sig = signer.sign(tbs, issuer_key.private_key);
    if (!sig.success) {
        return CertificateResult<Crl>::failure(sig.error_message);
    }

    AlgorithmIdentifier alg{};
    alg.signature = SignatureAlgorithmId::Ed25519;

    Crl crl{};
    crl.version = 2;
    crl.signature = alg;
    crl.issuer = issuer_;
    crl.this_update = this_update_;
    crl.next_update = next_update_;
    crl.revoked = entries_;
    crl.outer_signature = alg;
    crl.signature_value = sig.data;
    crl.tbs_der = tbs;
    std::vector<std::vector<uint8_t>> cert_fields;
    cert_fields.push_back(tbs);
    cert_fields.push_back(der::encode_sequence(der::encode_oid(*oid_for_signature(alg.signature))));
    cert_fields.push_back(der::encode_bit_string(ByteSpan(sig.data.data(), sig.data.size())));
    crl.der = der::encode_sequence(der::concat(cert_fields));

    return CertificateResult<Crl>::ok(std::move(crl));
}

CertificateResult<Crl> CrlBuilder::build_ed25519(const crypto::Lockey::KeyPair &issuer_key) const {
    return build(issuer_key);
}

} // namespace lockey::cert
